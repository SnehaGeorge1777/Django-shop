...
class OrderView(mixins.ListModelMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin,
    ...
    def get_queryset(self):
        ...
        if "secret" in self.request.parser_context["kwargs"]:
            queryset = queryset.filter(token=self.request.parser_context["kwargs"]["secret"]).order_by('-updated_at')
        elif not self.request.customer.is_visitor:
