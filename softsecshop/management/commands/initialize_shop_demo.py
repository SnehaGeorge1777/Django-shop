...
class Command(BaseCommand):
    help = _("Initialize the workdir to run the demo of softsecshop.")
    ...
    def handle(self, verbosity, *args, **options):
        ...
        if os.path.isfile(initialize_file):
            self.stdout.write("Initializing project softsecshop")
            call_command('makemigrations', 'softsecshop')
            call_command('migrate')
            ...
            self.stdout.write("Project softsecshop already initialized")
            call_command('migrate')

    def create_polymorphic_subcategories(self):
        from cms.models.pagemodel import Page
        from shop.management.commands.shop import Command as ShopCommand
        from softsecshop.models import Commodity, SmartCard, SmartPhoneModel
